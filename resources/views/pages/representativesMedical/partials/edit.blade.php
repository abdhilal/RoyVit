                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <x-form action="{{ route('representativesMedical.update', $representative->id) }}" method="PUT"
                            ...
                            <x-input name="name" label="{{ __('Name') }}" :model="$representative" required col="12" />

                            <div class="col-sm-6 col-md-6">
                                <label>{{ __('Working dates') }}</label>
                                <div class="border rounded mb-2 p-2" style="max-height: 180px; overflow-y: auto;">
                                    @foreach ($files as $file)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="file_ids[]"
                                                id="file_{{ $file->id }}" value="{{ $file->id }}"
                                                @checked(collect(old('file_ids', $representative->files->pluck('id')->toArray()))->contains($file->id))>
                                            <label class="form-check-label" for="file_{{ $file->id }}">{{ $file->month_year }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                                    ...
                                    @foreach ($areas as $area)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="area_ids[]"
                                                ...
                                                @checked(collect(old('area_ids', $representative->areas->pluck('id')->toArray()))->contains($area->id))>
